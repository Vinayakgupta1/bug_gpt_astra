...
class Scanner:
    ...
    async def start(self):
        try:
            ...
            # Define a helper function for serialization
            ...
            self.results = convert_to_serializable(self.results)
            serialized_results = json.dumps(convert_to_serializable(self.results))  # Convert to JSON
    ...
    async def _save_results_to_db(self):
        ...
        try:
            # Use the helper function to serialize results
            def convert_to_serializable(obj):
                """Convert datetime objects to ISO format for JSON serialization"""
                if isinstance(obj, datetime):
                    return obj.isoformat()
            ...
            scan_result = ScanResult(scan_id=self.scan_id, results=json.dumps(convert_to_serializable(self.results)))
            async with async_session() as session:
                async with session.begin():
                    session.add(scan_result)
                await session.commit()
    ...
    def get_results(self):
        ...
        return convert_to_serializable(self.results)
